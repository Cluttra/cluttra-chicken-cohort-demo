...
const two = new Intl.NumberFormat("hu-HU",{maximumFractionDigits:2});
...
export default function Home(){
  ...
  const stockScale=Math.max(5000,Math.ceil(Math.max(1,size)/5000)*5000);
  const feedScale=maxFeed/Math.max(1,size)*stockScale;
  ...
  return <main>
    ...
    <section className="hero" id="top"><div><p className="eyebrow">Digitális iker koncepció · baromfitartás</p><h1>Egy kohorsz.<br/><em>Minden működési jelzés.</em></h1><p className="intro">Prezentációra kész, heti tervezési nézet az állomány alakulásáról, takarmányigényéről, munkaerő-szükségletéről, egészségügyi eseményeiről és tojástermeléséről.</p></div><div className="heroStatus"><span>Modellállapot</span><b>DEMÓ ÜZEMMÓD</b><small>Szemléltető feltételezések · élő IoT-adatkapcsolat nélkül</small></div></section>
    <section className="workspace"><aside className="controls"><div className="sectionTitle"><span>01</span><div><b>Kohorszbeállítások</b><small>Minden mező azonnal újraszámol</small></div></div><label>Faj<input value="Csirke" readOnly/></label><label>Kohorsz típusa<select value={type} onChange={e=>setType(e.target.value as CohortType)}><option value="Broiler">Brojler</option><option value="Layer">Tojó</option><option value="Breeder">Tenyészállomány</option></select></label><label>Telepített állomány<input type="number" min="1" step="100" value={size} onChange={e=>setSize(Number(e.target.value))}/></label><label>Kezdőnap<input type="date" value={start} onChange={e=>setStart(e.target.value)}/></label><label>Szcenárió<select value={scenario} onChange={e=>setScenario(e.target.value)}><option>Alapeset</option><option>Magas inputköltség</option><option>Hatékonysági eset</option></select></label><details className="assumptionPanel" open><summary><span>Modellfeltételezések</span><small>{type==="Broiler"?"Brojler":type==="Layer"?"Tojó":"Tenyész"} profil</small></summary><div className="assumptionGrid"><div><span>Ciklus</span><b>{profile.days} nap</b></div><div><span>Céltesttömeg</span><b>{two.format(profile.weight)} kg</b></div><div><span>Takarmányprofil</span><b>{type==="Broiler"?`${two.format(profile.feedCycle)} kg / állat`:`${two.format(.35)}–${two.format(type==="Layer"?.8:1.05)} kg / hét`}</b></div><div><span>Mortalitás</span><b>{one.format(profile.broodMort*100)}% + {one.format(profile.laterMort*100)}%</b></div><div><span>Munkaerő</span><b>{profile.labour} óra / 1000 / hét</b></div><div><span>Állatorvosi keret</span><b>{two.format(profile.vet)} EUR / állat</b></div>{type!=="Broiler"&&<><div><span>Tojásindulás</span><b>{profile.layStart}. hét</b></div><div><span>Éves tojáscél</span><b>{profile.eggs} db / állat</b></div></>}</div><p>Szemléltető alapértékek; a végleges paramétereket a telepi technológia és az állatorvosi program határozza meg.</p></details></aside>
      ...
      <div className="chartCard"><div className="cardHead"><div><p className="eyebrow">Heti igénygörbe</p><h2>Takarmányigény és állomány-alakulás</h2></div><div className="legend"><span><i className="dot blue"/>Takarmány kg · bal tengely</span><span><i className="dot green"/>Élő állomány · jobb tengely</span></div></div><div className="chartFrame"><div className="yAxis leftAxis" aria-hidden="true">{axisTicks.map(t=><span key={t}>{qty.format(feedScale*t)}</span>)}</div><div className="chart">{rows.map((r,i)=><div className="barCol" key={r.week} title={`${r.week}. hét · ${qty.format(r.feedKg)} kg takarmány · ${qty.format(r.closing)} élő állat`}><div className="survival" style={{height:`${Math.max(2,r.closing/stockScale*88)}%`}}/><div className="feedbar" style={{height:`${Math.max(2,r.feedKg/feedScale*88)}%`}}/><span>{rows.length<=12||i%4===0?`${r.week}. h.`:""}</span></div>)}</div><div className="yAxis rightAxis" aria-hidden="true">{axisTicks.map(t=><span key={t}>{qty.format(stockScale*t)}</span>)}</div></div><div className="chartHint">Abszolút skála · {qty.format(stockScale)} állatos tervezési kapacitás. Az oszlopok fölé állva megjelennek a heti értékek.</div></div></div>
    </section>
    ...
    <footer><p><b>Cluttra Demo · döntéstámogató prototípus.</b> Az egészségügyi és oltási események szerkeszthető helyőrzők; a felelős állatorvosnak és a helyi hatósági előírásoknak kell jóváhagyniuk őket.</p><p>Forrásalap: Aviagen Ross műszaki portál · Hy-Line tartástechnológiai útmutató</p></footer>
  </main>;
}
